...
class Barman(threading.Thread):
    def __init__(self,pic,bar):
        ...
        print(f"[{self.__class__.__name__}] prêt pour le service!")
    async def nettoyer(self):
        while commandes!=[] or pic.contenu!=[]:
            await asyncio.sleep(1.5)
            print(f"[{self.__class__.__name__}] Je nettoie le comptoir")
    ...
    async def f(self):
        async with asyncio.TaskGroup() as tg:
            task3 = tg.create_task(self.preparer())
            task4 = tg.create_task(self.nettoyer())
    def run(self):
        asyncio.run(self.f())
...
async def main(): 
    async with asyncio.TaskGroup() as tg:
      ...
      task2 = tg.create_task(Steve.servir())

async def second():
    async with asyncio.TaskGroup() as tg:
        task3 = tg.create_task(Bob.preparer())
        task4 = tg.create_task(Bob.nettoyer())
